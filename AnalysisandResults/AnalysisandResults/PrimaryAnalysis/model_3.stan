...
//Hyperparameter boundaries
  ...
  real<lower=0, upper=2500> beta_s;
  real<lower=0, upper=2500> rho_s;
  real<lower=0> etatr_s;

//Parameter boundaries  
  vector<lower=0, upper=2500>[S] beta; //this corresponds to beta^2 in the mathematical models
  vector<lower=0, upper=2500>[S] rho;  //this corresponds to rho^2 in the mathematical models 
...
//We assign the appropriate parameters to each trial
 ...
 for (n in 1:N){
    ...
    B[n]= X[n]/beta_n[n] + (q[n]+ (eta_n[n]-1)*std[n])/rho_n[n];
    mean_two[n]=  B[n]/A[n]; 
...
generated quantities {
    ...
    for (n in 1:N){
     ...
     for (n in 1:N){
       log_lik[n] = normal_lpdf(r[n] | X[n], sqrt(beta_n[n])) + normal_lpdf(r[n] | q[n] + (eta_n[n]-1)*std[n], sqrt(rho_n[n]));
         }   
}
